...
/** 获取设备类型 i18n 键 */
export function getDeviceTypeKey(type: DeviceType): string {
    return `peer.deviceType.${type}`
}

/** 获取设备状态 i18n 键 */
export function getPeerStatusKey(status: PeerStatus): string {
...
/** 获取设备显示名称 i18n 键（当设备名为空时） */
export function getPeerDisplayNameKey(): string {
    return 'history.unknownDevice'
}
