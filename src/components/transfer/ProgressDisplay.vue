      ...
      <v-row dense>
        <v-col cols="6">
          ...
          <div class="text-subtitle-2">
            {{ formatSize(task?.transferredBytes || 0) }} / {{ formatSize(task?.file.size || 0) }}
          </div>
        </v-col>
        <v-col cols="6">
          <div class="text-body-2 text-grey">传输速度</div>
          <div class="text-subtitle-2">{{ formatSpeed(task?.speed || 0) }}</div>
        </v-col>
        <v-col cols="6">
          <div class="text-body-2 text-grey">预估剩余时间</div>
          <div class="text-subtitle-2">{{ formatTime(task?.estimatedTimeRemaining) }}</div>
        </v-col>
        <v-col cols="6">
          <div class="text-body-2 text-grey">传输模式</div>
          <div class="text-subtitle-2">{{ task?.mode === 'local' ? '本地网络' : '云盘中转' }}</div>
        </v-col>
      </v-row>
